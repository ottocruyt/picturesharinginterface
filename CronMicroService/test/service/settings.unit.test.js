...
describe("Settings unit tests", function () {
  ...
  describe("check", function () {
    it("should return true if value is the same", function () {
      const valueCorrect = settings.check("frequency", 1);
      expect(valueCorrect).to.equal(true);
    });
    it("should return false if value is the different", function () {
      const valueCorrect = settings.check("frequency", 2);
      expect(valueCorrect).to.equal(false);
    });
  });
});
